...
def main():
    ...
    lora_config = config['transfer_learning']['lora']
    ...
    if args.epochs:
        lora_config['training']['num_train_epochs'] = args.epochs
    if args.batch_size:
        lora_config['training']['per_device_train_batch_size'] = args.batch_size
    if args.learning_rate:
        lora_config['training']['learning_rate'] = args.learning_rate
    if args.max_length:
        lora_config['training']['max_length'] = args.max_length
    ...
    logger.info(f"LoRAファインチューニング開始")
    logger.info(f"ベースモデル: {lora_config['model_name']}")
    ...
    try:
        ...
        train_dataset = fine_tuner.prepare_dataset(
            [lora_config['data']['instruction_template'].format(**item) 
             for item in train_instructions],
            max_length=lora_config['training']['max_length']
        )
        ...
        # パラメータ情報の表示
        param_info = fine_tuner.get_trainable_params()
        logger.info(f"訓練可能パラメータ: {param_info['trainable_params']:,}")
        logger.info(f"全パラメータ: {param_info['all_params']:,}")
        logger.info(f"訓練可能パラメータ比率: {param_info['trainable_percentage']:.2f}%")
        
        # テスト生成
        test_prompt = "### Instruction:\nPythonでHello Worldを出力するコードを書いてください。\n\n### Response:\n"
        generated_text = fine_tuner.generate(test_prompt, max_length=100)
