...
def merge_lora_model(
    ...
    # マージ済みモデルの保存（MLX用にSafeTensors形式）
    logger.info("SafeTensors形式で保存中...")
    model.save_pretrained(
        output_path,
        safe_serialization=True,  # MLX用にSafeTensors形式で保存
        max_shard_size="2GB"
    )
    ...
    # LM Studio用の追加設定ファイルを作成
    import json
    
    # generation_config.jsonを作成
    generation_config = {
        "do_sample": True,
        "temperature": 0.7,
        "top_p": 0.9,
    ...
    logger.info(f"generation_config.json を作成: {generation_config_path}")
